...
for t=1:25       %start
        ...
        output_angles(right_hand_angles, left_hand_angles, temp_left_joint_angles, temp_right_joint_angles);
        pause(time);
...
for m=1:no_of_steps
    for t=1:40    %left leg as swing one
        if (t>-1) && (t<11)       %dsp1
            ...
            xa = -3;
            height_ankle = 0;
            centre_pelvis(3) =h*cos(atan((centre_pelvis(2)-5)/h));
    ...
    for t=1:40                     %right leg as swing leg
        if (t>-1) && (t<11)        
            centre_pelvis(1) = t/5;
            centre_pelvis(2) =3*(t)/10+5;
            xa = -3;
